---
title: "Project 3 - Group 6 Main Script (Xgboost Model + Baseline)"
author: "Jingwen Wang and Ziyi Liao (credit to Chengliang Tang, Tian Zheng)"
...
# Time for constructing training features= 131.276 s (note: this number varies due to the computer memory)
```
...
```{r run_cv_model1, message=FALSE, warning=FALSE}
# Initial Baseline
if(run.cv){
...
```{r xgb_cv}
if(run.xgb){
  xgb.param.all <- xgb.parameters(feat_train,label_train,5)
...
if(run.xgb){
  ...
  tm_train <- system.time(fit_train <- train.xgboost(feat_train, label_train,par_best))
...
# this varies due to computer memory: Time for super-resolution= 3.23 s (one-picture)
```
